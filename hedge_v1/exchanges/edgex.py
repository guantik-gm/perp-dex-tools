        ...
        while retry_count < max_retries:
            try:
                ...
                if last_order_price is not None and abs(self.round_to_tick(order_price) - last_order_price) / last_order_price > order_price_diff_rate:
                    ...
                    return OrderResult(success=True, order_id=last_order_id, price=last_order_price)
                ...
                if order_info:
                    if order_info.status == 'CANCELED':
                        # 这里取消直接返回上层处理，在新对冲模式下edgex+lighter的下单间隔需要尽可能靠近对冲策略决策时间，这里一直cancel+retry的话lighter价格偏移会越来越大
                        # if retry_count < max_retries - 1:
                            # retry_count += 1
                            # continue
                        # else:
                            # return OrderResult(success=False, error_message=f'Order rejected after {max_retries} attempts')
                        msg = f"Order canceled due to unmatch price, order {last_order_id} status should be [CANCELED]"
                        self.logger.log(msg, "INFO")
                        return OrderResult(success=True, order_id=last_order_id, price=last_order_price)
