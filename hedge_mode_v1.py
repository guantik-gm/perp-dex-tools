...
Examples:
    ...
    parser.add_argument('--exchange', type=str, required=True,
                        help='Exchange to use (grvt, or edgex)')
...
def validate_exchange(exchange):
    ...
    supported_exchanges = ['grvt', 'edgex']
...
def get_hedge_bot_class(exchange):
    ...
    try:
        if exchange.lower() == 'grvt':
            from hedge_v1.hedge_mode_grvt import HedgeBot
            return HedgeBot
        elif exchange.lower() == 'edgex':
            from hedge_v1.hedge_mode_edgex import HedgeBot
            return HedgeBot
        else:
